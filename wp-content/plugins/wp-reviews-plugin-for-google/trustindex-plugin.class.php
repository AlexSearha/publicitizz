...
'replied' => isset($review['reply']) && $review['reply'] ? 1 : 0
]);
if($wpdb->last_error)
{
...
$chosed_platform = new self($force_platform, $file_path, "do-not-care-10.2.2", "do-not-care-Widgets for Google Reviews", "do-not-care-Google");
...
if($version >= 10.0 && count($wpdb->get_results('SHOW COLUMNS FROM `'. $table_name .'` LIKE "replied"')) == 0)
{
$wpdb->query('ALTER TABLE `'. $table_name .'` ADD replied TINYINT(1) NOT NULL DEFAULT \'0\' AFTER date');
}
}
